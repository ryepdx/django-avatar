...
def add(request, extra_context=None, next_override=None,
    ...
    if request.method == "POST" and 'avatar' in request.FILES:
        if upload_avatar_form.is_valid():
            ...
            avatar.save()
            messages.add_message(request, messages.INFO, "Successfully uploaded a new avatar.")
...
def delete(request, extra_context=None, next_override=None, *args, **kwargs):
    ...
    if request.method == 'POST':
        if delete_avatar_form.is_valid():
            ...
            messages.add_message(request, messages.INFO, "Successfully deleted the requested avatars.")
            return HttpResponseRedirect(next_override or _get_next(request))
    return render_to_response(
